...
def row_minutes(row):
    ...
    try:
        ...
        # (mit eventuellem Komma oder Punkt) in eine Float-Stundenzahl
        net_hours_decimal = float(net_hours_str)
        # Gib die Stunden als ganze Minuten zurück
        # (wie der Rest der Funktion erwartet)
        return int(net_hours_decimal * 60)
    except ValueError:
        # Wenn das Feld 'Netto_Stunden'
        # leer, nicht vorhanden oder fehlerhaft ist,
        # fallback zur alten Methode der
        # Neuberechnung aus Einzelzeiten.
        pass
...
def generate_employee_report(base_dir, month, emp_id=None):
    ...
    if full_name:
        user_folder = full_name.split()[-1]  # Gets Lastname

        folder = base_dir / "data" / "working" / "ungeprueft" / user_folder
    ...
    if target_file is None:
        print("❌ Keine passende ungepruefte Datei gefunden.")
        return None
    ...
    # Filename: YYYY-MM_report_ID_Name.txt
    out_path = out_dir / (
        "%s_report_%03d_%s.txt" % (month, emp_id_from_file, nick)
    )
